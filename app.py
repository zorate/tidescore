...
def dev_login():
    """Disable dev login during maintenance"""
    flash('Development login is disabled during maintenance.', 'info')
    return redirect(url_for('home'))
...
@app.route('/health')
def health_check():
    return jsonify({'status': 'healthy', 'app': 'TideScore', 'mode': 'maintenance'})
# === ADMIN ROUTES ===
@app.route('/admin')
@admin_required
def admin_dashboard():
